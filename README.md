# nut standalone для бюджетных Ippon с уведомление в Telegram
Устанавливаем nut:
```
...
```
Сам скрипт `/etc/nut/telegram.sh`:
```
#!/bin/sh
...
# Подставляем свои данные бота и номер чата\группы для получения уведомлений. Вместо пробелов нужно использовать %20
telegram="curl -s "https://api.telegram.org/bot{BOT_ID}:{BOT_TOKEN}?chat_id={CHAT_ID}&text=""
# Перебираем возможные значения переданного скрипту параметра и шлём сообщение
case $1 in
        onbatt)
                        ${telegram}"🟢🟢🟢%20Работа%20ИБП%20от%20АКБ."
                ;;
        online)
                        ${telegram}"✅✅✅Работа%20ИБП%20от%20сети%20восстановлена."
                ;;
        lowbatt)
                        ${telegram}"🔴🔴🔴Низкий%20заряд%20АКБ%20на%20сервере.%20Аварийное%20завершение%20работы!"
                ;;
        replbatt)
                        ${telegram}"⚠️ ⚠️ ⚠️ Требуется%20замена%20АКБ%20ИБП%20на%20сервере!"
                ;;
        commok)
                        ${telegram}"✅Связь%20с%20ИБП%20восстановлена."
                ;;
        commbad)
                        ${telegram}"⛔Потеряна%20связь%20с%20ИБП!"
                ;;
        nocomm)
                        ${telegram}"⛔⛔⛔Нет%20связи%20с%20ИБП!"
                ;;
esac
```
Делаем его исполняем, затем перезапускаем демон nut и мониторинг:
```
root@proxmox:~# chmod + x /etc/nut/telegram.sh
root@proxmox:~# systemctl restart nut-server.service
root@proxmox:~# systemctl start nut-monitor
```
